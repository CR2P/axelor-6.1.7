...
@Entity
@Table(name = "BASE_BATCH", indexes = { @Index(columnList = "alarm_engine_batch"), @Index(columnList = "base_batch"), @Index(columnList = "mail_batch"), @Index(columnList = "meta_file"), @Index(columnList = "crm_batch"), @Index(columnList = "sale_batch"), @Index(columnList = "invoice_batch"), @Index(columnList = "accounting_batch"), @Index(columnList = "supplychain_batch"), @Index(columnList = "presta_shop_batch"), @Index(columnList = "presta_shop_batch_log") })
public class Batch extends AuditableModel {
	...
	@ManyToOne(fetch = FetchType.LAZY, cascade = { CascadeType.PERSIST, CascadeType.MERGE })
	private PrestaShopBatch prestaShopBatch;

	@ManyToOne(fetch = FetchType.LAZY, cascade = { CascadeType.PERSIST, CascadeType.MERGE })
	private MetaFile prestaShopBatchLog;
	...
	public PrestaShopBatch getPrestaShopBatch() {
		return prestaShopBatch;
	}

	public void setPrestaShopBatch(PrestaShopBatch prestaShopBatch) {
		this.prestaShopBatch = prestaShopBatch;
	}

	public MetaFile getPrestaShopBatchLog() {
		return prestaShopBatchLog;
	}

	public void setPrestaShopBatchLog(MetaFile prestaShopBatchLog) {
		this.prestaShopBatchLog = prestaShopBatchLog;
	}
